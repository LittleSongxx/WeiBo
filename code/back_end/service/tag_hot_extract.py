...
async def update_hot_data(tag_task_id: str, mongo_db: AsyncIOMotorDatabase) -> TagHot:
    ...
    try:
        ...
        one_day_dict = {
            "data_time": list(one_day.to_dict()[tag].keys()),
            "data_count": list(one_day.to_dict()[tag].values()),
        }
        one_month_dict = {
            "data_time": list(one_month.to_dict()[tag].keys()),
            "data_count": list(one_month.to_dict()[tag].values()),
        }
        three_month_dict = {
            "data_time": list(three_month.to_dict()[tag].keys()),
            "data_count": list(three_month.to_dict()[tag].values()),
        }
