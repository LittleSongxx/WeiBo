...
def get_post_detail(weibo_id: str, tag_comment_task_id: str):
    response = requests.get(
        "http://127.0.0.1:8001/weibo_curl/api/statuses_show?weibo_id={weibo_id}".format(
            weibo_id=weibo_id
        )
    )
    ...
    if "data" in weibo_dict and "result" in weibo_dict["data"]:
        ...
        print(f"Warning: API返回数据格式异常: {weibo_dict}")
        ...
        try:
            if item["analysis_status"] != "SUCCESS":
                ...
                if task.state == "PROGRESS":
                    ...
                    mydb.update_one(
                        {"_id": item["_id"]}, {"$set": {"analysis_status": task.state}}
                    )
        except Exception:
