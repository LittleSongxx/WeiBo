...
@gen.coroutine
def weibo_web_curl(
    curl_aim: SpiderAim, retry_time=settings.RETRY_TIME, with_cookie=True, **kwargs
):
    """
    ...
    for epoch in range(retry_time):  # 最多进行retry_time次的请求尝试
        request = RequestBuilder(**kwargs).make_request(
        ...
        try:
            ...
            try:
                ...
                charset_match = charset_pattern.search(
                ...
                if not charset_match:
                    ...
                    charset = (
                        charset_match.group().strip('"').strip("'")
                    )  # 移除可能的引号
                # print(response.body.decode(charset))
                # 用于寻找html中title部分的正则匹配pattern
                title_pattern = re.compile(r"<title>.*</title>")
                html_title = title_pattern.search(
                    response.body.decode(charset, errors="ignore")
                ).group(0)
                if (
                    html_title == "<title>新浪通行证</title>"
                    or html_title == "<title>登录 - 新浪微博</title>"
                ):
        ...
        if http_code == 302 or http_code == 403:  # Cookie 失效
            return {
                "error_code": 3,
                "errmsg": "Invalid cookie: {}".format(request.headers.get("Cookie")),
            }
